...
class TrainConfig:
    ...
    def __post_init__(self):
        if self.use_reward_model:
            checkpoint_name = f"IQL/{self.env}/fn_{self.feedback_num}/mode_{self.mode}/at_{self.attention_type}/n_{self.noise}/e_{self.epochs}/s_{self.seed}"
        else:
            checkpoint_name = f"IQL/{self.env}/seed_{self.seed}_{str(uuid.uuid4())[:8]}"
...
def train(config: TrainConfig):
    ...
    if config.use_reward_model:
        model = reward_model.RewardModel(config, None, None, None, None, None, dimension)
        base_path = os.getcwd() + "/reward_model"
        path = os.path.join(base_path, config.env)
        path = os.path.join(
            path,
            "fn_" + str(config.feedback_num),
            'mode_' + config.mode,
            'at_'+ config.attention_type,
            "n_" + str(config.noise),
    ...
    # Load BC pretrained model if mode is BC-P
    if config.mode == "BC-P":
        bc_path = os.path.join("bc_model", "BC", config.env, f"seed_{config.seed}", "bc_policy.pt")
        if os.path.exists(bc_path):
            actor.load_state_dict(torch.load(bc_path))
            print(f"[INFO] BC pretrained model loaded: {bc_path}")
        else:
            print(f"[WARNING] BC pretrained model not found: {bc_path}")

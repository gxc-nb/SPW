...
│ └── utils_env.py # Environment & dataset utils
│
├── Reward_learning/ # Reward model
│ ├── learn_reward.py # Training entry for reward model
│ ├── reward_model.py # Reward model architecture
│ └── reward_utils.py # Helper functions for reward learning
│
├── configs/ # YAML configs (bc.yaml, iql.yaml, reward.yaml)
├── dataset/ # MetaWorld & DMC datasets
├── preference_datasets/ # Optional human preference data
├── scripts/ # Shell scripts (e.g., example.sh)
